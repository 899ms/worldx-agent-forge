...
const DESIGN_MAX_TOKENS = parseInt(process.env.ORCHESTRATOR_DESIGN_MAX_TOKENS || "8192", 10);
...
export async function designWorld(userPrompt) {
  ...
  const rawResult = await chatJSON({
    ...
    userMessage: prompt,
    temperature: 0.7,
    maxTokens: Number.isFinite(DESIGN_MAX_TOKENS) && DESIGN_MAX_TOKENS > 0 ? DESIGN_MAX_TOKENS : 8192,
  });
